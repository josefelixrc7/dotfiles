...
* `SUBDIRS = lib src . test`: hay que tener cuenta que el orden en el que coloquemos estos directorios debe de ser ordenado, debido a que no se puede ir a compiar a `test` cuando no se tiene `src`
